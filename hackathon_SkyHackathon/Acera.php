...
class Acera
{
    ...
    const STARTING_VIRTUAL_SCORE = 25;
...
//        $sql = "INSERT INTO User (id, first_name, last_name, username) VALUES (29, 'adam', 'szewe', 'adamszewe')";
    ...
    {
        $sql = "select credits from Bet where user_id = :userId";
        $parameters = array(
            ':userId' => $userId
        );
        $statement = $this->db->query($sql, $parameters);
    ...
    public function getBetCredits($userId)
    {
        $sql = "select  bet_credits from Bet where user_id = :userId";
        $parameters = array(
            ':userId' => $userId
        );
        $statement = $this->db->query($sql, $parameters);
        $row = $statement->fetch(PDO::FETCH_ASSOC);
        if (isset($row["bet_credits"])) {
            return $row["bet_credits"];
        } else {
            return false;
        }
    }
    ...
    public function updateCredits($userId, $value)
    {
        $fields = array(
            'user_id',
            'credits'
        ...
        $result = $this->db->update("Bet", "user_id", $fields, $data);
    }


    public function updateBetCredits($userId, $betValue)
    {
        $fields = array(
            'user_id',
            'bet_credits'
    ...
    public function upsertBet($userId, $betting) {
        if ($this->getBetCredits($userId) !== false) {
            // do nothing - we'll update the bet     
        } else {
            // insert a new bet
            $sql = "insert into Bet (user_id, credits, bet_credits) values (:userId, :credits, :betCredits)";
            $parameters = array(
                'userId' => $userId,
                'credits' => Acera::STARTING_VIRTUAL_SCORE,
                'betCredits' => 0
            );
        ...
        if ($betting > $currentCredits) {
            // betting is impossible - credit too low
            echo "you can't bet!!";
            return false;
        } else {
            // betting is possible, let's go
            $currentBet = $this->getBetCredits($userId);
            $currentBet += $betting;
            $currentCredits -= $betting;
            $this->updateCredits($userId, $currentCredits);
            $this->updateBetCredits($userId, $currentBet);
        }
    }
    ...
    public function upsertUserTeam($userId, $team) {
        if ($this->getUserTeam($userId)) {
            // the user already selected a team
            $fields = array(
                'user_id',
                'team'
            );
            ...
            $result = $this->db->update("UserTeam", "user_id", $fields, $data);
        } else {
            // otherwise insert
            $sql = "insert into UserTeam (user_id, team) values (:userId, :team)";
            $parameters = array(
                ':userId' => $userId,
                'team' => $team
            );
    ...
    public function getUserTeam($userId) {
        $sql = "select  team from UserTeam where user_id = :userId";
        $parameters = array(
            'userId' => $userId
        );
        $statement = $this->db->query($sql, $parameters);
